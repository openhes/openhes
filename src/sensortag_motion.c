...
// =========================================================================
// CC2650 SensorTag — 9-Axis Motion (MPU-9250) reader via BlueZ D-Bus API
//
...
// -------------------------------------------------------------------------
// Find a GATT characteristic by UUID using ObjectManager
// -------------------------------------------------------------------------
static GDBusProxy *
...
// -------------------------------------------------------------------------
// Convert raw 18-byte motion data.
//
// 9 signed 16-bit LE integers:
//   [0..5]  Gyroscope  X, Y, Z
//   [6..11] Accelerometer X, Y, Z
//   [12..17] Magnetometer X, Y, Z
//
// Scale factors (matching Python script):
//   Gyro: value * (500.0 / 65536.0)  →  °/s
//   Accel: value * (8.0 / 32768.0)   →  G
//   Mag:   value * 1.0               →  µT
// -------------------------------------------------------------------------
static void
convert_motion(const guchar *data, gsize len,
    double *gyro, double *accel, double *mag)
{
	if (len < 18)
		return;
...
// -------------------------------------------------------------------------
// Periodic timer -- read motion data and print
// -------------------------------------------------------------------------
static gboolean
on_read_motion(gpointer user_data)
{
	(void) user_data;
	...
	if (conf_proxy) {
		...
		GVariant *args = g_variant_new("(ay@a{sv})",
		    g_variant_new_fixed_array(
		        G_VARIANT_TYPE_BYTE, disable, 2, 1),
		    g_variant_new_array(
...
main(void)
{
	...
	dev_proxy = g_dbus_proxy_new_sync(connection,
	    ...
	    NULL, &error);
	if (!dev_proxy) {
		g_printerr("Device proxy failed.\n"
		           "  Error: %s\n", error->message);
		g_error_free(error);
		g_object_unref(connection);
		return 1;
	...
	data_proxy = find_char_by_uuid(connection,
	    DEVICE_PATH, MOTION_DATA_UUID, &error);
	if (!data_proxy) {
	...
	conf_proxy = find_char_by_uuid(connection,
	    DEVICE_PATH, MOTION_CONF_UUID, &error);
	if (!conf_proxy) {
	...
	printf("Enabling 9-Axis Motion Sensors...\n");
	{
		...
		GVariant *args = g_variant_new("(ay@a{sv})",
		    g_variant_new_fixed_array(
		        G_VARIANT_TYPE_BYTE, enable, 2, 1),
		    g_variant_new_array(
		        G_VARIANT_TYPE_VARDICT, NULL, 0));
		g_dbus_proxy_call_sync(conf_proxy, "WriteValue", args,
		    G_DBUS_CALL_FLAGS_NONE, 5000, NULL, &error);
	...
	// ---- 6. Periodic reading (every 500 ms) ----
	// Print header once; live updates use ANSI escapes to overwrite
	printf("\n\n\n\n");
	printf("--- MOTION DATA ---\n");
	...
	printf("\033[4A");
	g_timeout_add(500, on_read_motion, NULL);
