...
// =========================================================================
// CC2650 SensorTag — Ambient Light (OPT3001) reader via BlueZ D-Bus API
//
// Discovers the light service characteristics by UUID, enables the sensor,
// and reads illuminance (lux) every second.
// =========================================================================
...
// CC2650 Ambient Light Sensor (OPT3001) UUIDs
#define LIGHT_DATA_UUID "f000aa71-0451-4000-b000-000000000000"
#define LIGHT_CONF_UUID "f000aa72-0451-4000-b000-000000000000"
...
// -------------------------------------------------------------------------
// Convert raw 2-byte sensor data to lux.
//
// OPT3001 modified 16-bit floating-point format:
//   Bits 0-11:  Mantissa (fractional result)
//   Bits 12-15: Exponent
//
//   Lux = Mantissa * (0.01 * 2^Exponent)
// -------------------------------------------------------------------------
static double
convert_lux(const guchar *data, gsize len)
{
	if (len < 2)
		return 0.0;

	guint16 raw = (guint16)data[0] | ((guint16)data[1] << 8);

	guint16 mantissa = raw & 0x0FFF;
	guint16 exponent = (raw >> 12) & 0x000F;

	return (double)mantissa * (0.01 * (double)(1 << exponent));
}

// -------------------------------------------------------------------------
// Periodic timer callback -- read light data and print
// -------------------------------------------------------------------------
static gboolean
on_read_light(gpointer user_data)
{
	(void) user_data;
	...
	if (len >= 2) {
		double lux = convert_lux(bytes, len);
		printf("Ambient Light: %.2f Lux   \r", lux);
	} else {
...
cleanup(void)
{
	printf("\nDisabling sensor to save battery...\n");
...
main(void)
{
	...
	// ---- 4. Discover light service characteristics ----
	printf("Discovering ambient light sensor...\n");

	data_proxy = find_char_by_uuid(connection,
	    DEVICE_PATH, LIGHT_DATA_UUID, &error);
	if (!data_proxy) {
		g_printerr("Failed to find light data characteristic: %s\n",
	...
	conf_proxy = find_char_by_uuid(connection,
	    DEVICE_PATH, LIGHT_CONF_UUID, &error);
	if (!conf_proxy) {
		g_printerr("Failed to find light config characteristic: %s\n",
		    error ? error->message : "unknown");
		g_clear_error(&error);
		g_object_unref(data_proxy);
		g_dbus_proxy_call_sync(dev_proxy, "Disconnect", NULL,
	...
	// ---- 5. Enable light sensor ----
	printf("Enabling Ambient Light Sensor...\n");
	{
		guchar enable = 0x01;
		GVariant *args = g_variant_new("(ay@a{sv})",
		    g_variant_new_fixed_array(
	...
	printf("\nReading brightness levels (Ctrl+C to stop):\n");
	g_timeout_add(1000, on_read_light, NULL);
